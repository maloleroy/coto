...
\noindent with $\alpha'_i = \alpha_i \sqcup \mathbf{0}$,
$\delta_j = \beta(A)_j \sqcup \beta(C)_j$, and
$\gamma'_k = \gamma_k \sqcup \mathbf{0}$. More formally, the merging formula is:
$$\text{fm}(A, C) = (\{(\alpha'_0, a_0), ..., (\alpha'_{l-1}, a_{l-1}), (\delta_0, b_0), ..., (\delta_{n-1}, b_{n-1}), (\gamma'_0, c_0), ..., (\gamma'_m, c_{m-1})\}, \emptyset)$$
...
For a child that occurs in only one operand, its missing amplitude in the other operand is
$\mathbf{0}$. Consequently, its coefficient in the result is $\alpha_i \sqcup \mathbf{0}$
or $\gamma_k \sqcup \mathbf{0}$, rather than $\alpha_i$ or $\gamma_k$ alone. This widening is
necessary: retaining a unique coefficient exactly would force that contribution to be present
and would therefore fail to contain the operand in which the branch is absent.

The formula above assumes a non-additive context as well as non-additive operands. This condition
matters for additive diagrams with interval coefficients. Separate edges are independent
contributions in the interval evaluation; combining coefficients can reintroduce a correlation and
cancel uncertainty that those contributions encode. The general additive implementation therefore
uses a compact uniform enclosure: every amplitude of the merged node is bounded by the join of the
two nodes' global enclosures. This is less precise than the formula above, but it is monotone under
arbitrary additive parents and avoids an exponentially growing multiset of branches. Once a merge
has occurred, subsequent unitary gates are enclosed using their row-norm bound rather than a
correlation-sensitive structural rewrite.
